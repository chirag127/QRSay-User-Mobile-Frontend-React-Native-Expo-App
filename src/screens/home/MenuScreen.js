...
const MenuScreen = ({ navigation, route }) => {
    ...
    const filterMenu = () => {
        // Ensure menu is defined and is an array
        let filtered = Array.isArray(menu) ? menu : [];

        // Apply search filter
        if (searchQuery && typeof searchDishes === "function") {
            const searchResults = searchDishes(searchQuery);
        ...
        if (
            selectedFilter !== "all" &&
            typeof filterDishesByType === "function"
        ) {
            const typeResults = filterDishesByType(selectedFilter);
        ...
        if (selectedCategory && Array.isArray(filtered)) {
    ...
    const handleCategorySelect = (categoryId) => {
        ...
        if (sectionListRef.current && categoryId && Array.isArray(menu)) {
            const sectionIndex = menu.findIndex(
                (section) => section && section.categoryId === categoryId
            );
    ...
    const handleAddToCart = (dish) => {
        addToCart({
            ...
            restaurantName:
                restaurant?.restaurantName || currentRestaurant?.restaurantName,
        });
    };
    ...
    const getDishQuantityInCart = (dishId) => {
        if (!Array.isArray(cartItems) || !dishId) return 0;
        const cartItem = cartItems.find((item) => item && item._id === dishId);
    ...
    const renderSectionHeader = ({ section }) => (
        <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>
                {section?.categoryName || "Unknown Category"}
            </Text>
    ...
    // Render error state
    if (error && (!menu || !Array.isArray(menu) || menu.length === 0)) {
        return (
            <ErrorDisplay
            ...
            >
                <Text style={styles.categoriesButtonText}>
                    {selectedCategory && Array.isArray(categories)
                        ? categories.find((c) => c && c.id === selectedCategory)
                              ?.name || "Categories"
                        : "Categories"}
                </Text>
                <Ionicons
